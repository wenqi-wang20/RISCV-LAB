module hazard_handler(
  ...
  // signals from mmu
  input wire mmu_mem_sel_i,
  input wire mmu_if_sel_i,

  // signals to IF, MEM
  output reg if_mem_access_en_o,
  output reg mem_mem_access_en_o
);
  ...
  always_comb begin
    // unused for now
    if_mem_access_en_o = 1'b1;
    mem_mem_access_en_o = 1'b1;
